...
export function showAnimationBox(
  data: CellPopData,
  width: number,
  height: number,
) {
...
function createStackedBar(
  svgBase: d3.Selection<d3.BaseType, unknown, HTMLElement, any>,
  data: CellPopData,
  dimensionsAnimation: AnimationDimensions,
) {
  const countsMatrixAnimation =
    data.countsMatrix as CountsMatrixValueAnimation[];
  ...
  for (const entry of countsMatrixAnimation) {
    const currVal = rowValsCounter.filter((r) => r.row === entry.row)[0]
      .counter;
  ...
  const svg = svgBase
    ...
    .attr(
      "transform",
      "translate(" +
        dimensionsAnimation.marginLeft +
        "," +
        dimensionsAnimation.marginTop +
        ")",
    );
  ...
  svg.append("g").attr("class", "axisleft").call(d3.axisLeft(y));
  ...
  svg
    ...
    .data(countsMatrixAnimation, function (d) {
      return d.row + ":" + d.col;
    })
    .enter()
    .append("rect")
  ...
  d3.select(".animate")
    ...
    .attr(
      "transform",
      "rotate(90, " +
        dimensionsAnimation.moveWidth +
        ", " +
        dimensionsAnimation.moveTop +
        ")",
    ); //650, 750
