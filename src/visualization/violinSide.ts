...
export function renderLeftViolin(
  data: CellPopData,
  ...
  const svg = d3
    ...
    .attr(
      "transform",
      "translate(" +
        dimensions.barLeft.offsetWidth +
        "," +
        dimensions.barLeft.offsetHeight +
        ")",
    )
  ...
  const width =
    ...
    dimensions.barLeft.margin.left -
    dimensions.barLeft.margin.right;
  const height =
    dimensions.barLeft.height -
    dimensions.barLeft.margin.top -
    dimensions.barLeft.margin.bottom;
  ...
  // Add y-axis
  const x = d3.scaleLinear().range([width, 0]).domain([0, upperbound]);
  ...
  svg
    ...
    .attr(
      "transform",
      "translate(0," + (height + dimensions.barLeft.margin.bottom) + ")",
    )
    .selectAll("text")
